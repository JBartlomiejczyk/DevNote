...
- [x] 2.3 Navigate to `/notes` while logged in — see list of existing notes � 617663b
...
- [x] 3.7 "Przejdź do notatek" link navigates to `/notes` � 617663b
...
- 2026-06-29 — testy wykonane na branchu `note-management`, potwierdzone commitami `617663b` (P2-P4 manual) i `44d10c3` (P1 manual).
- `/notes`: lista, empty-state, badge, nawigacja do `/edit/{id}` — OK.
- `/edit/{id}`: prefill, status Draft po wejściu, re-classify aktualizuje ten sam rekord (bez duplikatu), wynik inline + link do `/notes` — OK.
- Granice auth: brak dostępu bez logowania + redirect z return-url; obce/błędne ID przekierowuje do `/notes` — OK.

## Review Addendum (2026-06-29)

- `Services/WizardStateService.cs` zostało celowo rozszerzone o `Reset()` i `LoadFromNote(...)`, aby utrzymać spójną hydrację/reset stanu kreatora między trasami `/` i `/edit/{id}`.
